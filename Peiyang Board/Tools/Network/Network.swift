...
struct Network {
    ...
    static func requestWithFormData(urlString: String, method: Method, headers: [String: Any] = [:], parameters: [String: Any] = [:], dataPath: [String: Data] = [:], completion: @escaping (Result<(Data, HTTPURLResponse), Failure>) -> Void){
        ...
        for (key, value) in dataPath {
            body.appendString(string: "--\(boundary)\r\n")
            body.appendString(string: "Content-Disposition: form-data; name=\"\(key)\"; filename=\"\(arc4random())\".png\r\n") //此處放入file name，以隨機數代替，可自行放入
